...
| GET | `/internal/whatsapp/groups` | Same HMAC auth. Lets the gateway pull its watch list, so the owner's toggle in the CRM is the single source of truth. Each row carries `last_message_at` — the newest message the CRM has stored for that group, and so the gateway's resume point after a disconnect. Null for a group that has never delivered anything, which the gateway reads as "no gap to close" and falls back to its own age window, so switching a group on does not drag its history in. The watermark lives here rather than on the gateway because the gateway is the half that sleeps: a laptop suspends and the host restores its filesystem from the repo on every deploy. |
| GET | `/whatsapp/session` | Owner only — gateway connection state and the pairing QR. The code is withheld once rotated or if the gateway has gone quiet, so a spent code is never rendered. |
| POST | `/internal/whatsapp/session` | Gateway reporting its socket state and QR. HMAC-signed like the ingest webhook: an unsigned caller could otherwise put its own pairing code in front of the owner. |
| GET | `/whatsapp/ingestion-status` | Owner only — groups connected, queue depth, failures in 24h, listings produced, reposts merged, whether extraction credentials are configured at all, and whether an extraction loop has actually reported in recently (`extractor_running`, `extractor_seen_at`, `extractor_note`), and `stalled` — claims whose worker died mid-batch, counted separately from `processing` so a batch legitimately in flight is never called stuck. Configured and running are separate facts: a key can be set with nothing running it, and the symptom — a queue that never drains — is identical. |
...
| POST | `/whatsapp/reprocess/{id}` | Owner only — requeue a message. Raw bodies are stored before parsing precisely so a prompt fix can be replayed over history, which is why the feed offers this on every message and not only failed ones: a message that "succeeded" and was read wrongly never reaches the failed state. |
| POST | `/whatsapp/reprocess-failed` | Owner only — requeue every message that gave up, returning `{"requeued": n}`. Resets `attempts`, without which a requeued message falls outside the claim filter and is never picked up again. Covers `failed` rows and claims stranded in `processing` past the stall window; leaves `pending` rows and in-flight claims alone. |
